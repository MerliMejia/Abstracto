...
public:
  static BakedImageBasedLightingData
  bakeOrLoadCache(const ImageBasedLightingBakeSettings &settings) {
    BakedImageBasedLightingData bakedData;
    if (tryLoadBakeCache(settings, bakedData)) {
      return bakedData;
    }

    const EnvironmentSource environmentSource = loadEnvironmentSource(settings);
    bakedData.environment = bakeEnvironmentMap(settings, environmentSource);
    bakedData.irradiance = bakeIrradianceMap(settings, environmentSource);
    bakedData.prefiltered = bakePrefilteredMap(settings, environmentSource);
    bakedData.brdfLut = bakeBrdfLut(settings);

    fixupCubemapSeams(bakedData.environment);
    fixupCubemapSeams(bakedData.irradiance);
    fixupCubemapSeams(bakedData.prefiltered);
    saveBakeCache(settings, bakedData);
    return bakedData;
  }
...
private:
  ...
  buildBakeCacheKey(const ImageBasedLightingBakeSettings &settings) {
    ...
    if (!settings.environmentHdrPath.empty()) {
      ...
      if (std::filesystem::exists(hdrPath)) {
        ...
        key << ";hdrMtime=" << static_cast<long long>(hdrTimestamp);
      }
    } else {
  ...
  static bool tryLoadBakeCache(const ImageBasedLightingBakeSettings &settings,
    ...
    BrdfDimensions brdfDimensions{};
    if (!readCubemapMipChain(stream, bakedData.environment) ||
        !readCubemapMipChain(stream, bakedData.irradiance) ||
        !readCubemapMipChain(stream, bakedData.prefiltered) ||
        !readBinary(stream, brdfDimensions.width) ||
        !readBinary(stream, brdfDimensions.height) ||
        !readFloatVector(stream, bakedData.brdfLut)) {
      return false;
    }
  ...
  static void saveBakeCache(const ImageBasedLightingBakeSettings &settings,
    ...
    writeCubemapMipChain(stream, bakedData.environment);
    writeCubemapMipChain(stream, bakedData.irradiance);
    writeCubemapMipChain(stream, bakedData.prefiltered);
